...
FFTDIAS::FFTDIAS(double f, double z0,int N, int M)
	:f(f),z0(z0),N(N),M(M)
{
	lamda = C_Speed / f;
	k = 2 * Pi * f / C_Speed;
...
void FFTDIAS::SetInput(std::complex<double> ** ExIn, std::complex<double> ** EyIn)
{
	for (int i = 0; i < N2; i++)
		for (int j = 0; j < M2; j++)
		{
			...
			if ((i < N) && (j < M))
			{
				Ex0[i + (N-1)/2][j + (M-1)/2] = ExIn[i][j];
				Ey0[i + (N-1)/2][j + (M-1)/2] = EyIn[i][j];
			}
		}

}

void FFTDIAS::Setds(double ds1)
{
	ds = ds1;
}

void FFTDIAS::SetPropagationParas(double _xp, double _yp, double _zp, double _theta_rou, double _phi_rou)
{
	xp = _xp;
	yp = _yp;
	zp = _zp;
...
void FFTDIAS::output(std::complex<double>** Ex, std::complex<double>** Ey, std::complex<double>** Ez, std::complex<double>** Hx, std::complex<double>** Hy, std::complex<double>** Hz)
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < M; j++)
...
void FFTDIAS::MulComp(const double r1, const double i1, const double r2, const double i2, double & outR, double & outI)
{
	outR = r1 * r2 - i1 * i2;
	outI = r1 * i2 + r2 * i1;
}

std::complex<double> FFTDIAS::MulComp(std::complex<double> & In, double r2, double i2)
{
	return std::complex<double>(In.real() * r2 - In.imag() * i2,
		In.real() * i2 + r2 * In.imag());
}

double FFTDIAS::InserVal(const double x0, const double yUp, const double y0, const double yDown)
{
	if ((yUp == y0) || (yDown == y0))
	{
		return y0;
...
void FFTDIAS::axisRotate()
{
	//构建旋转矩阵
	Matrix4D Ma1,Ma2,Ma;
	Ma1.setRow(0, Vector3D(cos(theta_rou), 0, sin(theta_rou)));
	Ma1.setRow(1, Vector3D(0, 1, 0));
	Ma1.setRow(2, Vector3D(-sin(theta_rou), 0, cos(theta_rou)));
	Ma1.setRow(3, Vector3D(0,0,0));
	Ma2.setRow(0, Vector3D(cos(phi_rou), sin(phi_rou), 0));
	Ma2.setRow(1, Vector3D(-sin(phi_rou), cos(phi_rou), 0));
	Ma2.setRow(2, Vector3D(0, 0, 1));
	Ma2.setRow(3, Vector3D(0, 0, 0));
